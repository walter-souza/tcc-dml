...
dataloaders['evaluation'] = torch.utils.data.DataLoader(datasets['testing'], num_workers=opt.kernels, batch_size=opt.bs, shuffle=False)
...
dataloaders['training'] = torch.utils.data.DataLoader(datasets['training'], num_workers=opt.kernels, batch_size=opt.bs, shuffle=False)
opt.n_classes  = len(dataloaders['training'].dataset.avail_classes)


# data_iterator = dataloaders['training']
# print("DatasetLen:", len(dataloaders['training'].dataset.labels))
...
print("DEVICE", torch.cuda.get_device_properties(opt.device))
for epoch in range(opt.n_epochs):
    ...
    for i,out in enumerate(data_iterator):
        ...
        # encodings = opt.tokenizer(out['inputs'], return_tensors='pt', truncation=True, padding='max_length')
        ...
        ### Compute Embedding
        # inputs = torch.FloatTensor(out['inputs']).to(opt.device)
        input_ids      = out['input_ids'].to(opt.device)
        attention_mask = out['attention_mask'].to(opt.device)

        embeds = model(input_ids, attention_mask=attention_mask)
        # embeds = model(opt, input_ids, attention_mask)
        
        # print("input.shape:", input_ids.shape, "embeds.shape:", embeds.shape)
        # input()

        ### Compute Loss
        loss_args['batch']          = embeds
        loss_args['labels']         = class_labels
        loss_args['f_embed']        = model.last_linear
...
print(opt.save_path+'/training_summary.txt')
