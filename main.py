...
for epoch in range(opt.n_epochs):
    ...
    for i,out in enumerate(data_iterator):
        class_labels, input_, input_indices = out

        ### Compute Embedding
        input_      = input_.to(opt.device)
        model_args = {'x':input_.to(opt.device)}
        ...
        # print("\tinput.shape:", input_.shape, "embeds.shape:", embeds.shape)
        # input()
